...
- (void)enqueueSelectedDateRange:(NSDate *)date_begin END:(NSDate *) date_end;
...
- (void)initialize
{
    ...
    _gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierPersian];
...
#else
    _minimumDate = [self.formatter dateFromString:@"1373-01-01"];
...
- (void)enqueueSelectedDateRange:(NSDate *)date_begin END:(NSDate *) date_end
{
    ...
    if (![_selectedDates containsObject:date_begin]) {
        [_selectedDates addObject:date_begin];
    }
}
...
- (void)requestBoundingDatesIfNecessary
{
    if (!_needsRequestingBoundingDates) {
        ...
        _minimumDate = [self.dataSourceProxy minimumDateForCalendar:self]?:[self.formatter dateFromString:@"1373-01-01"];
